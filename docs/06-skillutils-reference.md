...
Creates a labeled, filled, bordered box as a `picture`, shifted to `position`. This is the fundamental building block for flowchart and system-diagram nodes.
...
When generating a standalone `.asy` file that uses these utilities, inline all three functions at the top of the file:
